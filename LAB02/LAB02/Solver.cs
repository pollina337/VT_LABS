...
namespace LAB02
{
    ...
    return $"Месяц {month} - {season}";
        ...
        {
    // Работаем с абсолютным значением числа, чтобы обрабатывать отрицательные числа
    int absNumber = Math.Abs(number);
    
    // Преобразуем число в строку для перебора цифр
    string numberStr = absNumber.ToString();
    
    // Инициализируем минимальную и максимальную цифры
    // Берем первую цифру как начальное значение
    int minDigit = int.Parse(numberStr[0].ToString());
    int maxDigit = int.Parse(numberStr[0].ToString());
    ...
    return $"Число: {number}, минимальная цифра: {minDigit}, максимальная цифра: {maxDigit}";
}
    ...
    {
        return $"Количество Элементов Больших {filter} = 0";
    }
